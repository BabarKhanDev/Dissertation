...
Measuring the memorability of an image has been done by many people 
% References: Isola etc
There are two approaches to measuring the memorable regions of an image. First, like Khosla in LaMem, you could split an image into many regions and use a model designed to predict the memorability of an image, predict the memorability of that region, then compose these memorabilities into a memorability map. Or secondly, like in VISCHEMA, you could perform a memorability game like above, except when a user says they remember an image, ask them what part of the image caused them to remember it. Both of these methods have their drawbacks. In the former, the memorability of an image may not be equal to that image within another image. That is to say, the environment in which we see an image, intuitively, must have some impact on the memorability of it. In the latter, the way that people indicate what caused them to remember the image may be fallible, if users draw boxes around objects, they may or may not intend for the edges to be selected. In either case the result is the same, a base of image, memorability map pairs in which supervised learning can occur on.
...
Assuming that we have a databse of image and memorability map pairs as described above, we can use an image to image deep convolutional model to learn a mapping from the image to the memorability map. There are many of these that exist, but fundementally they all perform the same task and can be evaluated the same. Given an image they need to predict what the corresponding memorability map looks like. We can evaluate the quality of these solutions in a number of ways. The L1 and L2 loss can describe the absolute and mean square error between two equally sized images, 
...
LaMem contains 60,000 images, compared to the 1,600 in VISCHEMA Plus, this significant increase should allow any deep learning models that we use to generalise much more easily and would greatly reduce our risk of overfitting. Unfortunately, while they show in their paper a correlation between region memorability and image memorability, 
% double check that this is a correlation
I'm not entirely convinced that this holds in every case, and have chosen to use the VISCHEMA Plus dataset. Akagunduz et al. successfully measure the memorable regions of images, rather than a score per image which I believe to be more robust. The VICHEMA Plus dataset contains 1600 image to VMS mappings, 1280 of these compose the training portion and the remaining 320 the validation portion, due to the small size I will have to make use of tools such as data augmentation to prevent overfitting. Each VMS map represents the memorable regions of its corresponding image.

\textbf{Ethical Concerns}

% NOT GOOD, THIS NEEDS TO BE ABOUT PEOPLE
...
I aim to create and evaluate a range of deep convolutional image to image model to learn a mapping of VISCHEMA Plus images to their corresponding VMS labels. Our models will all be evaluated against the requirements detailed in \ref{tab:RequirementCaptureTable}. State of the art models would include, autoencoders, GANs, and Diffusion models. As the three of them produce images in different ways I think it would be valuable to compare their performance in the same task. I will run three experiments where I train a UNet autoencoder, a GAN, and a diffusion model to produce a VMS label given an image from the dataset. I hope to be able to find the strengths and weaknesses of each of these systems. For each model I will experiment with network parameters and training hyperparameters to fine tune models that produce the best output.
...
The UNet is an Autoencoder model presented in \cite{ronneberger2015unet}. 
Its original creation was for the purpose of medical image segmentation. A Diagram of the model with the best parameters can be seen in figure \ref{fig:autoencoderDiagram}, the model is composed of two blocks, an encoder and a decoder. 
...
In the higher resolution layers the convolutional filters are able to see fine grain features, such as the leaves in a tree. In the lower resolution layers convolution filters are able to see more coarse features, at the deepest layer a forest of trees. 
...
If we consider an image $ I $ to be a 3d matrix with width $ w_1 $, height $ h_1 $, and depth $ c_1 $ and a corresponding label $ L $ to be a 3d matrix with width $ w_2 $, height $ h_2 $, and depth $ c_2 $. Then an autoencoder works by taking image $ I $ and producing an output with dimmensions equal to $ L $. We will call the ground truth label $ L_{real} $ and the autoencoder output $ L_{pred} $. It's been shown in 
% reference to L1 loss being able to produce higher accuracy images in pix2pix paper
that training with the L1 loss allows models to create sharper images. When training a UNet autoencoder with the L1 loss the equation would be:

\begin{equation}
    \label{eqn:UNet Loss}
    \texttt{UNet Loss} = \frac{1}{w \times h \times c} \sum_{i = 0}^{w_2} \sum_{j = 0}^{h_2} \sum_{k = 0}^{c_2}(L_{real}[i,j,k], L_{pred}[i,j,k] ) 
\end{equation}

This loss describes the average absolute difference between each corresponding pixel in the ground truth label and the predicted label. Each convolutional layer within the UNet has adjustable weights $w$ that can be adjusted to produce a better output. The training loop uses backpropagation of this loss to adjust the model weights. To adjust a given weight $w_i$ calculate $ \frac{\delta L}{\delta w_i} $ and use gradient descent to adjust it. Please see algorithm \ref{ALG:Autoencoder} for the full training loop.
...
textbf{Generative Adversarial Networks}

% presented by ian goodfellow and is composed of a generaor and a discriminator

The Generative Adversarial Network is an image synthesis model described by Goodfellow et al \cite{goodfellow2014generative}. In this model there are two adversarial networks, a generator, $ G $, and a discriminator, $ D $,. I will describe an image from the learning distribution as real, and one created by the generator as fake. The goal of the generator is to, given a latent space vector $ z $, produce an image that the discriminator thinks is real. The goal of the discriminator is to take an image, either real or fake, and say which it is. In its vanilla form the loss of each network is relative to the performance of the other making it impossible to see if the networks have converged. The loss function described in \cite{goodfellow2014generative} is a minimax game, where G is trying to maximise the probability of D making a mistake, and D is trying to minimise it. More formally: The loss function of G is such that it is trying to maximise equation \ref{eqn:Vanilla Generator Loss} and the loss function of D is such that it is trying to maximise equation \ref{eqn:Vanilla Discriminator Loss}. Condtional generative adversarial networks can create images using a latent space vector and a condition parameter. For example, the class of the image to produce, simply pass this parameter into both the generator and the discriminator. In this case the discriminator outputs the probability that $ L $ is real and that meets the condition.
...
% Implementation and testing will be discussed in the results analysis section

\chapter{Old Methodology}
...
\begin{figure}[h]
    ...
    \includegraphics[width=\linewidth]{UNet Diagram}
    \caption{UNet Autoencoder Diagram}
    \label{fig:autoencoderDiagramOLD}
\end{figure}
